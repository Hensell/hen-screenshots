...
A local plugin for Codex, Claude Code, and Cursor can turn a folder of captures into a styled series, render a preview, and export PNGs by language plus an editable Hen project. It reuses the web editor’s templates and scene engine, with native rendering through Konva and Skia. Requires Node.js 22.12+ and a one-time dependency installation.

See [plugin setup and commands](plugins/hen-screenshots/README.md) and the [design spec](plugins/hen-screenshots/skills/create-screenshots/references/design-spec.md). Build it with `npm run plugin:build`; create a local ZIP with `npm run plugin:pack`. Version 0.2.2 adds Cursor manifests and local installation instructions. Download it from the [GitHub Release](https://github.com/Hensell/hen-screenshots/releases/tag/plugin-v0.2.2), this repository, or the website. The [OpenAI Plugins Directory](https://chatgpt.com/plugins/plugins_6aa32bb05be881918e9fa402a5a1cde6) currently provides v0.2.1. Anthropic review is pending (checked September 14, 2026). Cursor uses local installation while its marketplace submission, received on September 14, 2026, is pending review; it is not yet listed. All hosts share the same renderer, including project background-image support.

The [AI agents guide](https://screenshots.hensell.dev/agents/) includes a built ZIP download, one-time setup, copyable Codex, Claude Code, and Cursor examples, and the workflow for opening generated projects in the studio. It is also available in [Spanish](https://screenshots.hensell.dev/es/agents/) and [Brazilian Portuguese](https://screenshots.hensell.dev/pt-br/agents/). Website builds package the current plugin version automatically; `npm run plugin:pack -- --web` prepares the download for local development.
...
**Translate with AI** works across the whole series. Add your language versions, choose which ones to translate, and start the queue. It processes one language at a time, translating each slide’s title and supporting text separately. Edited and reviewed captions are preserved unless you explicitly choose to replace them. Each completed language is saved as drafts and can be undone; stopping or a failed download keeps completed languages and leaves the unfinished language unchanged. You can continue with the remaining languages.

**Manual editing always works without a model download.** Optional AI translation uses OPUS-MT through Transformers.js in a dedicated WebAssembly worker; it does not use Chrome’s Translator API or a hosted inference service. Downloadable packs currently cover English, Spanish, French, and German. Pairs without English use two packs and translate through English. Other listed languages are marked for manual editing. Text inside uploaded images is not translated.

The download dialog explains the size before starting: roughly **112–120 MB per directional language pack**, plus shared runtime files on first use. Shared packs are counted once, and no download starts until you click **Start AI translation**. Files download from Hugging Face and jsDelivr; captions stay on the device. Packs are cached when browser storage permits, can be removed from the dialog, and are not included in project backups. Cache eviction or clearing site data can require another download. Performance depends on browser and hardware. Review machine translations before publishing, especially marketing headlines.
